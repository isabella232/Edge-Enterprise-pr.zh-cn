---
title: Microsoft Edge 语言支持
ms.author: kvice
author: dan-wesley
manager: srugh
ms.date: 06/29/2021
audience: ITPro
ms.topic: conceptual
ms.prod: microsoft-edge
ms.localizationpriority: medium
ms.collection: M365-modern-desktop
description: Windows 和 macOS 的 Microsoft Edge 显示语言支持。
ms.openlocfilehash: 5ffd39764b6fb96e7fd5dc3307a42fd16e24aee2f2475bb0eb4c2c6f066bf732
ms.sourcegitcommit: d44c0997ffe40d67421312ed96e7766da947eaa0
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 08/05/2021
ms.locfileid: "11726945"
---
# <a name="microsoft-edge-language-support"></a>Microsoft Edge 语言支持

本文介绍了如何使用 Microsoft Edge 设置向浏览器添加受支持的语言。 Microsoft Edge 在 Windows 上支持 84 种显示语言，在 macOS 上支持 81 种显示语言。

> [!NOTE]
> 本文适用于 Microsoft Edge 版本 78 或更高版本。

## <a name="add-a-supported-language-to-microsoft-edge"></a>将受支持的语言添加到 Microsoft Edge 中

使用以下步骤查看受支持语言的列表，然后将语言添加到 Microsoft Edge 中。

1. 在浏览器菜单栏上，单击**设置及更多**图标 (**...**) 或键入 **Alt+F** 以打开“设置及更多”菜单。
2. 在**设置**下，单击**语言**。
3. 在**语言**下，单击**添加语言**以查看可添加的语言列表。
4. 在**添加语言**列表中，你可以搜索语言（**搜索语言**）或使用滚动栏查找你想要添加的语言。
5. 找到该语言后，选中该语言的相应复选框，然后单击**添加**。 你添加的语言将显示在已为 Microsoft Edge 设置的**首选语言**列表的底部。 你访问的网站将以列表中受支持的第一种语言进行显示。

### <a name="change-preferred-languages"></a>更改首选语言

你可以重新排序语言首选项、让 Microsoft Edge 提供翻译，并以特定语言查看 Microsoft Edge。
要更改语言设置，请单击该语言旁边的省略号 (**...**) 以打开“更多操作”。
根据语言，你将看到以下一个或多个选项：

- 以这种语言显示 Microsoft Edge。
  - 在 Windows 上，选择此框，然后单击**重启**。
  - 在 macOS 上，Microsoft Edge 以系统的首选语言显示。 你可以在**系统首选项 > 语言和地区**中更改语言。 重启 Microsoft Edge。
- 主动以这种语言翻译页面。 仅当你启用**主动翻译非你所读语言的页面**时，此选项才可用。
- 移动到顶部
- 上移
- 删除

> [!NOTE]
> 你还可以根据首选语言设置来设置拼写检查选项（**检查拼写**）。

## <a name="foreign-language-translator-extension"></a>外语翻译扩展

除了在浏览器中使用外语外，还可以使用 [Translator for Microsoft Edge](https://www.microsoft.com/p/translator-for-microsoft-edge/9nblggh4n4n3) 扩展来翻译 60 多种语言的外语网页和所选文本。

> [!NOTE]
> 如果翻译扩展在安装后不起作用，请重启 Microsoft Edge。 如果扩展仍不起作用，请通过反馈中心提供反馈。

## <a name="supported-display-languages"></a>支持的显示语言

下表列出了 Microsoft Edge 支持的显示语言。

> [!NOTE]
> macOS 上 Microsoft Edge 不支持的语言由国家/地区后的星号 (*) 来指示。  

|                      语言                      |             国家/地区              |      代码      |
|----------------------------------------------------|-----------------------------------------|----------------|
|              南非荷兰语                             |              南非               |     af         |
|                 阿尔巴尼亚语                           |                 阿尔巴尼亚                 |     sq         |
|            阿姆哈拉语                                 |                埃塞俄比亚                 |     am         |
|               阿拉伯语                               |              沙特阿拉伯               |     ar         |
|               阿萨姆语                             |                  印度                  |     as         |
|          阿塞拜疆语                               |               阿塞拜疆，拉丁语         |    az          |
|                   孟加拉语                           |              印度*                     |     bn-IN      |
|                  巴斯克语                            |                  巴斯克语                 |     eu         |
|              波斯尼亚语                               |       波斯尼亚和黑塞哥维那，拉丁语     |       bs       |
|                保加利亚语                           |                保加利亚                 |     bg         |
|                 加泰罗尼亚语                            |                加泰罗尼亚语                  |     ca         |
|            中文                                 |              简体中文          |     zh-CN      |
|                                                    |             台湾，繁体中文         |     zh-TW      |
|                 克罗地亚语                           |                 克罗地亚                 |     hr         |
|                 捷克语                              |             捷克共和国              |     cs         |
|                  丹麦语                            |                 丹麦                 |     da         |
|                荷兰语                               |               荷兰               |     nl         |
|              英语                               |             英国              |     en-GB      |
|                                                    |              美国              |     en         |
|                 爱沙尼亚语                           |                 爱沙尼亚                 |     et         |
|               菲律宾语                             |               菲律宾               |     fil        |
|                 芬兰语                            |                 芬兰                 |     fi         |
|                  法语                            |                 加拿大                  |     fr-CA      |
|                                                    |                 法国                  |     fr         |
|                加利西亚语                            |                 加利西亚语                |     gl         |
|                      格鲁吉亚语                      |                 格鲁吉亚                 |     ka         |
|                  德语                            |                 德国                 |     de         |
|                   希腊语                            |                 希腊                  |     el         |
|                      古吉拉特语                      |                  印度                  |     gu         |
|                  希伯来语                            |                 以色列                  |     he         |
|                   印地语                            |                  印度                  |     hi         |
|                匈牙利语                           |                 匈牙利                 |     hu         |
|                     冰岛语                      |                 冰岛                 |     is         |
|               印度尼西亚语                           |                印度尼西亚                |     id         |
|                       爱尔兰语                        |                 爱尔兰                 |     ga         |
|                  意大利语                           |                  意大利                  |     it         |
|                  日语                          |                  日本                  |     ja         |
|                      埃纳德语                       |                  印度                  |     kn         |
|                哈萨克语                              |               哈萨克斯坦                |     kk         |
|                  高棉语                             |                柬埔寨                 |     km         |
|                      孔卡尼语                       |                  印度                  |     kok        |
|                   韩语                           |                  韩国                  |     ko         |
|                     老挝语                            |               老挝人民共和国               |     lo         |
|                  拉脱维亚语                           |                 拉脱维亚                  |     lv         |
|               立陶宛语                           |                立陶宛                |     lt         |
|             卢森堡语                          |               卢森堡                |     lb         |
|         马其顿语                                 |            北马其顿              |     mk         |
|                  马来语                             |                马来西亚                 |     ms         |
|              马拉雅拉姆语                             |                  印度                  |     ml         |
|               马耳他语                              |                  马耳他                  |     mt         |
|          毛利语                                     |               新西兰               |     mi         |
|               马拉地语                              |                  印度                  |     mr         |
|                尼泊尔语                              |               尼泊尔                     |     ne         |
|             挪威语                              |                 挪威，博克马尔语          |     nb         |
|                                                    |                 挪威，尼诺斯克         |     nn         |
|                 奥里亚语                               |                  印度                  |     or         |
|               波斯语                              |            伊朗                         |    fa          |
|                  波兰语                            |                 波兰                  |     pl         |
|                葡萄牙语                          |                 巴西                  |     pt-BR      |
|                                                    |                葡萄牙                 |     pt-PT      |
|               旁遮普语                              |                  印度                  |     pa         |
|                盖丘亚语                             |                  秘鲁                   |     quz        |
|                 罗马尼亚语                           |                 罗马尼亚                 |     ro         |
|                  俄语                           |                 俄罗斯                  |     ru         |
|            苏格兰盖尔语                         |             英国              |     gd         |
|            塞尔维亚语                                 |    波斯尼亚和黑塞哥维那，西里尔文*    |   sr-Cyrl-BA   |
|                                                    |                 塞尔维亚，西里尔文        |   sr-Cyrl-RS   |
|                                                    |                 塞尔维亚，拉丁语           |   sr-Latn-RS   |
|                 斯洛伐克语                             |                斯洛伐克                 |        sk      |
|                斯洛文尼亚语                           |                斯洛文尼亚                 |        sl      |
|              西班牙语                               |                 墨西哥                  |     es-MX      |
|                                                    |         西班牙，国际排序       |        es      |
|                  瑞典语                           |                 瑞典                  |     sv         |
|                   泰米尔语                            |                印度                    |     ta         |
|                       鞑靼语                        |                 俄罗斯                  |     tt         |
|                泰卢固语                              |                  印度                  |     te         |
|                  泰语                              |                泰国                 |     th         |
|                  土耳其语                           |                 土耳其                  |     tr         |
|                乌克兰语                           |                 乌克兰                 |     uk         |
|             乌尔都语                                   |       巴基斯坦伊斯兰共和国      |     ur         |
|              维吾尔语                                |                   中华人民共和国                   |     ug         |
|               巴伦西亚语                            |                  西班牙*                 | ca-Es-VALENCIA |
|            越南语                              |                 越南                 |     vi         |
|            威尔士语                                   |             英国              |     cy         |

---

## <a name="see-also"></a>另请参阅

- [Microsoft Edge 文档](./index.yml)
- [Microsoft Edge Enterprise 登录页面](https://aka.ms/EdgeEnterprise)