...
Microsoft Edge展台模式功能可以使用以下Windows 10命令行选项来调用数字/交互式标牌和公共浏览。
...
### <a name="kiosk-mode-download-files-on-exit"></a>出口上的展台模式下载文件

若要设置Microsoft Edge在关闭展台实例时删除下载的文件，必须配置以下两个组策略：
- [在退出时删除下载](./microsoft-edge-policies.md#kioskdeletedownloadsonexit) = 已启用
...
- **--kiosk-idle-timeout-minutes=**：在Microsoft Edge展台模式通过关闭浏览器重置用户会话之前，将时间 (从上一个用户活动更改为分钟) 。 注意：此标志在关闭后不会重启Microsoft Edge。 需要单独的技术（例如分配的访问权限或 Shell 启动）才能在空闲超时后自动重启 Edge。 将以下示例中的“值”替换为分钟数。
...
可用 Windows 10 上的[多应用指派访问](/windows/configuration/lock-down-windows-10-to-specific-apps)（相当于旧版 Microsoft Edge 的“普通浏览”展台模式类型）运行 Microsoft Edge。 若要使用多应用分配的访问权限配置 Microsoft Edge，请按照有关[如何设置多应用展台](/windows/configuration/lock-down-windows-10-to-specific-apps)的说明进行操作。  (Microsoft Edge稳定通道的 AUMID **Microsoft.MicrosoftEdge.Stable_8wekyb3d8bbwe！MSEDGE**) 。

将Microsoft Edge与多应用分配的访问权限配合使用时，可以将Microsoft Edge展台配置为使用[Microsoft Edge浏览器策略](./microsoft-edge-policies.md)来配置浏览体验以满足你的独特要求。
...
5. 键入名称以创建新的展台帐户，或从填充的下拉列表中选择现有帐户，然后选择 **Next**。
...
6. 在 **“选择展台”应用** 页上，选择**Microsoft Edge**，然后选择 **Next**。
